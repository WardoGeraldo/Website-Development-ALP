...
<div class="container">
    ...
    <form action="{{ route('admin.promo.update', ['id' => $promo['id']]) }}" method="POST" class="form-container" data-aos="fade-up">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label class="form-label">Promo Code <span style="color:#e76767">*</span></label>
            <input type="text" name="promo_code" class="form-control" value="{{ old('promo_code', $promo['promo_code'] ?? $promo['code']) }}" required maxlength="20" placeholder="Enter promo code">
            @error('promo_code')
        ...
        <div class="mb-3">
            ...
            <textarea name="description" class="form-control" rows="3" required placeholder="Describe the promo">{{ old('description', $promo['description']) }}</textarea>
            @error('description')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="discount" class="form-control" value="{{ old('discount', rtrim($promo['discount'], '%')) }}" required min="1" max="100" placeholder="e.g. 20">
            @error('discount')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
